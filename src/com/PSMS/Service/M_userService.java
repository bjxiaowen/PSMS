...
public interface M_userService {
	...
	* @date 2014-12-18 
	*/ 
	List<M_user> getAllUser();//获取全部user

	boolean addUser(M_user m_user);//插入用户数据
	...
	boolean checkUserIsExist(String User_name, String password);//检查登录的用户名和密码的正确性

	List<String> getAllStationName();//获得所有电站名称

	Integer getPsIdByName(String station_name);//根据电站名查询电站id

	Integer getRoleIdByName(String User_name);//根据用户名查询角色id

	boolean checkUserIsExist2(String User_name, String password);//检查该高级用户是否存在

	Integer getRoleIdByName2(String User_name);//得到高级用户的角色
